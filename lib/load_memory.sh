...
if [ -f "$STATE_FILE" ] && jq -e '.' "$STATE_FILE" >/dev/null 2>&1; then
    ...
    SUPEROC_USER_EMAIL=$(jq -r '.user.content // ""' "$STATE_FILE" 2>/dev/null | grep -i "email:" | head -1 | cut -d: -f2 | tr -d ' ')
    SUPEROC_USER_LOCATION=$(jq -r '.user.content // ""' "$STATE_FILE" 2>/dev/null | grep -i "location:" | head -1 | cut -d: -f2 | tr -d ' ')
    SUPEROC_USER_TIMEZONE=$(jq -r '.user.content // ""' "$STATE_FILE" 2>/dev/null | grep -i "timezone:" | head -1 | cut -d: -f2 | tr -d ' ')
